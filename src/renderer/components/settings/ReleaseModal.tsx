  ...
  return (
    <div className="mt-2 space-y-1">
      <div className="relative h-1 w-full rounded-full bg-surface-border overflow-hidden">
        <div
          className={`absolute left-0 top-0 h-full rounded-full transition-all duration-200 ${barColor} ${!isDone && !isError && !isCancelled && !isPaused ? 'animate-pulse' : ''}`}
          style={{ width: `${percent}%` }}
        />
      </div>
      <div className="flex items-center justify-between text-[10px] font-mono text-text-muted">
        <span>{label}</span>
        <span className={percentColor}>{isDone ? '100%' : `${percent}%`}</span>
      </div>
    </div>
  );
}
      ...
      <div className="px-5 py-4 space-y-5">
        ...
        {platformAsset && (
          <div className="rounded-lg border border-accent/20 bg-accent/5 px-4 py-3">
            ...
            {getDl(platformAsset.name) && <ProgressBar progress={getDl(platformAsset.name)!} />}
          </div>
        )}
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {(release.assets ?? []).map((asset) => (
                <div
                  key={asset.id}
                  className="px-3 py-2 rounded-lg border border-surface-border bg-base-900/50"
                >
                  <div className="flex items-center gap-3">
                  ...
                  {getDl(asset.name) && <ProgressBar progress={getDl(asset.name)!} />}
                </div>
              ))}
            </div>
          </div>
