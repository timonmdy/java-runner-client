    ...
    <div className="space-y-1.5">
      {items.map((item, i) => (
        <div key={i} className="flex items-center gap-2 group">
          <CheckToggle checked={item.enabled} onToggle={() => toggle(i)} />
          <input
            type="text"
            value={item.value}
...
function CheckToggle({ checked, onToggle }: { checked: boolean; onToggle: () => void }) {
  return (
    <button
      onClick={onToggle}
      ...
      className={[
        ...
        checked ? 'bg-accent border-accent' : 'bg-transparent border-surface-border hover:border-text-muted',
      ].join(' ')}
    >
      {checked && (
...
function RemoveButton({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} className="opacity-0 group-hover:opacity-100 transition-opacity text-text-muted hover:text-red-400">
