...
export function Tooltip({ content, children, delay = 400, side = 'top' }: Props) {
  ...
  const show = (e: React.MouseEvent<HTMLElement>) => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      let x = 0; let y = 0
      if (side === 'top')    { x = rect.left + rect.width / 2 - tw / 2; y = rect.top  - th - 6 }
      if (side === 'bottom') { x = rect.left + rect.width / 2 - tw / 2; y = rect.bottom + 6 }
      if (side === 'left')   { x = rect.left - tw - 6;                   y = rect.top  + rect.height / 2 - th / 2 }
      if (side === 'right')  { x = rect.right + 6;                       y = rect.top  + rect.height / 2 - th / 2 }
      // Clamp to viewport
