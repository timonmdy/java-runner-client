...
export function ProfileTab() {
  ...
  const handleSave   = async () => { await saveProfile(draft); setSaved(true); setTimeout(() => setSaved(false), 1800) }
  const handleDelete = async () => { await deleteProfile(draft.id); setShowDelete(false) }
    ...
    <>
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-5 space-y-6">
          <Section title="Name">
            <Input value={draft.name} onChange={e => update({ name: e.target.value })} placeholder="My Server"/>
          </Section>
            ...
            <div className="flex flex-wrap gap-2.5">
              {PROFILE_COLORS.map(c => (
                <button key={c} onClick={() => update({ color: c })} title={c}
                  ...
                  style={{ backgroundColor: c, boxShadow: draft.color === c ? `0 0 0 2px #08090d, 0 0 0 4px ${c}` : 'none', transform: draft.color === c ? 'scale(1.15)' : undefined }}/>
              ))}
            </div>
          </Section>
          <Section title="Danger Zone">
            <div className="rounded-lg border border-red-500/20 bg-red-500/5 px-4 py-3 flex items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-text-muted mt-0.5">Permanently removes this profile and all its configuration.</p>
              </div>
              <Button variant="danger" size="sm" onClick={() => setShowDelete(true)} className="shrink-0">Delete</Button>
            </div>
          </Section>
        </div>
      </div>
      <Dialog open={showDelete} title="Delete profile?"
        message={`"${draft.name}" will be permanently removed. This cannot be undone.`}
        confirmLabel="Delete" danger onConfirm={handleDelete} onCancel={() => setShowDelete(false)}/>
    </>
  )
}
