 ...
 *
 * IMPORTANT: All hooks must be declared before any conditional returns (React rules of hooks).
 */
...
function GeneralSection({ draft, update, running, color, onRestart }: {
  ...
  return (
    <div className="space-y-6">
      ...
      {running && (
        <div className="space-y-3">
          <h3 className="text-xs font-mono text-text-muted uppercase tracking-widest">Process Control</h3>
          <div className="flex items-center justify-between gap-4 rounded-lg bg-base-900 border border-surface-border px-3 py-2.5">
            <div>
              <p className="text-xs text-text-primary font-medium">Restart process</p>
              <p className="text-xs text-text-muted mt-0.5">Stop and immediately re-launch with current settings</p>
            </div>
            <Button variant="ghost" size="sm" onClick={onRestart} style={{ borderColor: color, color }}>
              Restart
            </Button>
          </div>
        </div>
  ...
  return (
    <div className="space-y-4">
      <Input label="JAR File" value={draft.jarPath} onChange={e => update({ jarPath: e.target.value })}
        placeholder="/path/to/server.jar" rightElement={<FolderBtn onClick={handlePickJar} />} />
