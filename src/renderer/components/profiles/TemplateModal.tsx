...
interface TemplateEntry { filename: string; template: ProfileTemplate }
...
export function TemplateModal({ open, onClose }: Props) {
  ...
  const load = async () => {
    ...
    setLoading(false)
    if (!res.ok || !res.data) { setError(res.error ?? 'Failed to load templates'); return }
    ...
    <Modal open={open} onClose={onClose} title="Profile Templates" width="lg">
      <div className="flex flex-col" style={{ height: 480 }}>
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b border-surface-border shrink-0">
          ...
          />
          <button onClick={load} disabled={loading} className="text-text-muted hover:text-accent transition-colors disabled:opacity-40 p-1">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-52 shrink-0 border-r border-surface-border overflow-y-auto py-2">
            ...
            {error && (
              <p className="text-xs text-red-400 font-mono px-4 py-6 text-center leading-relaxed">{error}</p>
            )}
            {templates && filtered.length === 0 && (
              <p className="text-xs text-text-muted font-mono px-4 py-6 text-center">No templates found.</p>
            )}
            {Object.entries(grouped).map(([cat, entries]) => (
              <div key={cat}>
                <p className="px-3 py-1.5 text-xs font-mono text-text-muted uppercase tracking-widest">{cat}</p>
                {entries.map(entry => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        ...
                        !compat      ? 'opacity-40 cursor-not-allowed text-text-muted' :
                    ...
                    >
                      {entry.template.name}
                      {!compat && <span className="ml-1 text-[10px] text-red-400">v{entry.template.templateVersion}</span>}
                    </button>
                  )
                })}
              </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4">
            {!selected ? (
              <div className="flex flex-col items-center justify-center h-full gap-3 text-text-muted">
                <VscPackage size={28} />
                <p className="text-xs font-mono text-center">Select a template to preview its configuration</p>
              </div>
            ) : (
              <div className="space-y-4">
                <div>
                  <h3 className="text-sm font-semibold text-text-primary">{selected.template.name}</h3>
                  <p className="text-xs text-text-muted mt-1 leading-relaxed">{selected.template.description}</p>
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {selected.template.tags.map(tag => (
                      <span key={tag} className="flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-mono bg-surface-raised border border-surface-border text-text-muted">
                        <VscTag size={9} />
                        {tag}
                      </span>
                    ))}
                ...
                <Section title="JVM Args">
                  {selected.template.defaults.jvmArgs.length === 0
                    ? <Empty />
                    : selected.template.defaults.jvmArgs.map((a, i) => <Pill key={i} value={a.value} enabled={a.enabled} />)
                  }
                </Section>
                ...
                <Section title="System Properties">
                  {selected.template.defaults.systemProperties.length === 0
                    ...
                    : selected.template.defaults.systemProperties.map((p, i) => (
                        <Pill key={i} value={`-D${p.key}${p.value ? `=${p.value}` : ''}`} enabled={p.enabled} />
                      ))
                  }
                </Section>
                ...
                <Section title="Program Args">
                  {selected.template.defaults.programArgs.length === 0
                    ? <Empty />
                    : selected.template.defaults.programArgs.map((a, i) => <Pill key={i} value={a.value} enabled={a.enabled} />)
                  }
                </Section>

                <div className="flex items-center gap-1.5 text-xs text-text-muted font-mono">
                  <LuShield size={11} />
                  Template version {selected.template.templateVersion} · Requires app {selected.template.minAppVersion}+
                </div>
              </div>
            )}
          </div>
...
function Pill({ value, enabled }: { value: string; enabled: boolean }) {
  return (
    <span className={['px-2 py-0.5 rounded font-mono text-xs border', enabled ? 'bg-base-950 border-surface-border text-text-secondary' : 'bg-base-950 border-surface-border text-text-muted opacity-50 line-through'].join(' ')}>
      {value}
    </span>
  )
}
...
function Empty()   { return <span className="text-xs text-text-muted font-mono italic">none</span> }
