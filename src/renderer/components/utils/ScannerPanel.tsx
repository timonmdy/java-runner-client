  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-surface-border bg-base-900/50 shrink-0 flex-wrap gap-y-2">
        ...
        {javaVisible && (
          <Button variant="danger" size="sm" onClick={() => setKillAllConfirm(true)}>Kill All Java</Button>
        )}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        {results === null && !scanning && (
          <EmptyState icon={<LuScanLine size={28} />} text='Click "Scan" to list all running processes' />
        )}
        {scanning && (
      ...
      {(['java', 'all'] as const).map(f => (
        <button
          ...
          className={[
            ...
            value === f ? 'bg-surface-raised text-text-primary' : 'text-text-muted hover:text-text-primary',
          ].join(' ')}
        >
...
function ExpandChevron({ expanded, onClick }: { expanded: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className="text-text-muted hover:text-text-primary transition-colors shrink-0">
      <svg
...
function DetailRow({ label, value, mono, wrap }: {
  label:  string
  value:  string
  mono?:  boolean
  wrap?:  boolean
