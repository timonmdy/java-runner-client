...
const loadLogs = (id: string, max: number): ConsoleLine[] => {
  try { const r = sessionStorage.getItem(SS_KEY(id)); return r ? (JSON.parse(r) as ConsoleLine[]).slice(-max) : [] } catch { return [] }
}
const saveLogs  = (id: string, lines: ConsoleLine[]) => { try { sessionStorage.setItem(SS_KEY(id), JSON.stringify(lines)) } catch { /* quota */ } }
const clearLogs = (id: string) => { try { sessionStorage.removeItem(SS_KEY(id)) } catch { /* ignore */ } }
...
const INITIAL_STATE: AppState = {
  profiles: [], activeProfileId: '', processStates: [], settings: null, consoleLogs: {}, loading: true,
}
...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    case 'INIT':
      return { ...state, profiles: action.profiles, activeProfileId: action.profiles[0]?.id ?? '', processStates: action.states, settings: action.settings, loading: false }
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      const [profiles, settings, states] = await Promise.all([
        window.api.getProfiles(), window.api.getSettings(), window.api.getStates(),
      ])
  ...
  useEffect(() => {
    ...
    return window.api.onConsoleLine((profileId, line) => {
      dispatch({ type: 'APPEND_LOG', profileId, line, maxLines: max })
    })
  ...
  const createProfile = useCallback((overrides: Partial<Profile> = {}) => {
    const p: Profile = {
      id:                  uuidv4(),
  ...
  return (
    <AppContext.Provider value={{
      state, activeProfile, setActiveProfile, saveProfile, deleteProfile, createProfile,
      startProcess, stopProcess, sendInput, clearConsole, saveSettings, isRunning,
    }}>
      {children}
    </AppContext.Provider>
  )
...
export const PROFILE_COLORS = [
  '#4ade80', '#60a5fa', '#f472b6', '#fb923c', '#a78bfa', '#34d399', '#fbbf24', '#f87171',
]
