/**
 * AppStore — lightweight shared state via React context + useReducer.
 */
import React, {
  createContext,
  useContext,
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!window.api) return
    const maxLines = state.settings?.consoleMaxLines ?? MAX_LINES_DEFAULT
    const unsub = window.api.onConsoleLine((profileId, line) => {
      dispatch({ type: 'APPEND_LOG', profileId, line, maxLines })
    })
    return unsub
  ...
  useEffect(() => {
    if (!window.api) return
    const unsub = window.api.onStatesUpdate((states) => {
  ...
  const createProfile = useCallback(() => {
    ...
    dispatch({ type: 'SET_PROFILES', profiles: [...state.profiles, newProfile] })
    dispatch({ type: 'SET_ACTIVE', id: newProfile.id })
    window.api.saveProfile(newProfile)
  }, [state.profiles])
