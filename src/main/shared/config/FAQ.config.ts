...
export const FAQ_TOPICS: FaqTopic[] = [
  {
    ...
    items: [
      ...
      { q: 'What is a "managed" (by JRC) process?', a: 'A "managed" process is a Java process that JRC started and is tracking. JRC spawns one process directly via Node.js and wraps it in a ManagedProcess object that tracks metadata. JRC captures all stdout/stderr output in real-time and monitors the process lifecycle.' },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { q: 'How do I use the REST API?', a: 'Enable the REST API in Settings. Then send HTTP requests to http://127.0.0.1:54321/api. Available endpoints: GET /status, GET/POST /profiles, GET /processes, POST /processes/:id/start, POST /processes/:id/stop, GET/PUT /settings.' },
    ],
  },
  {
